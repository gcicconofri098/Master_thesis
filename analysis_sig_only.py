...
for i in processes:
    ...
    pre_selection_signal = df[i].Count().GetValue()
    print("pre preselection signal, no weights: ", pre_selection_signal)
...
#    df[i] = df[i].Filter("new_discriminator[Jet1_index] > 0.996 && new_discriminator[Jet2_index] >0.98")
    ...
    post_selection_signal = df[i].Count().GetValue()

    print("post preselection signal, no weights: ", post_selection_signal)
...
for i in processes:
    histo2d[i] = df[i].Histo2D(
        ("Jet1 vs Jet2", "Jet1 vs Jet2; Jet1; Jet2", 25, 0, 1, 25, 0, 1),
        "Jet1_Selected_jets_discriminator",
        "Jet2_Selected_jets_discriminator",
    )
...
output_file = ROOT.TFile.Open("histograms_from_analysis_2d_discriminator_softdrop_cut_preselection_signal_only.root", "RECREATE")


for i in processes:

    output_file.WriteObject(h2_2[i], "h2_2_pres_only_" + str(i))
