...
for i in processes:
    ...
    df[i] = df[i].Filter("Jet1_Selected_jet_softdrop > 100 && Jet1_Selected_jet_softdrop < 150")
...
for i in processes:
    hist1[i] = df[i].Histo1D(
        (str(i), str(i) + "; Discriminator; Events", 20, 0, 220), "MET_pt"
    )
...
output_file = ROOT.TFile.Open("histograms_from_analysis_MET.root", "RECREATE")
