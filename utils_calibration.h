...
auto calibrate_pt_double_n_bins_softdrop(
                const ROOT::VecOps::RVec<float> &AK8_eta,
                const ROOT::VecOps::RVec<float> &AK8_pt,
                const ROOT::VecOps::RVec<float> &AK8_mass) {

        auto size_outer = AK8_eta.size();
        ROOT::VecOps::RVec<float> calibrated_mass (size_outer);

        for (int i = 0; i < size_outer; i++) {
            float scale = 1.0;
            float eta = AK8_eta[i];
            float pt = AK8_pt[i];
            float mass = AK8_mass[i];
